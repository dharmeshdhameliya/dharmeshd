...
def process_data(df):
    # Extend to 60 trading days
    all_results = {f'trading_day_{i + 1}': {'Yes': 0, 'No': 0, 'Highs': []} for i in range(60)}
    ...
    for _, row in df.iterrows():
        ...
        # Fetch data including previous day and next 70 days to ensure coverage of all trading days
        stock_data = fetch_stock_data(symbol, previous_day - timedelta(days=10), date + timedelta(days=70))
        ...
        if not stock_data.empty:
            try:
                ...
                # Process results for the next 60 trading days (filter trading days only)
                future_trading_days = stock_data.index[stock_data.index > date][:60]
    ...
    if 'logged_in' not in st.session_state or not st.session_state.logged_in:
        ...
        if uploaded_file is not None:
            ...
            if max_trading_day_yes:
                ...
                st.write("100% Yes results not achieved within 60 trading days")
